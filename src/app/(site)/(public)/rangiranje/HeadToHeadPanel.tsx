...
function ShooterPicker({
  ...
  return (
    <select
      ...
      aria-label={label}
      className="w-full rounded-lg border border-[var(--border)] bg-[var(--surface)] px-3 py-2.5 text-sm font-medium text-[var(--ink)] focus:outline-none focus:border-[var(--brand-primary)] cursor-pointer"
    >
      <option value="">{label}</option>
      {candidates
        .filter((c) => c.shooterId !== otherValue)
        .map((c) => (
          <option key={c.shooterId} value={c.shooterId}>
