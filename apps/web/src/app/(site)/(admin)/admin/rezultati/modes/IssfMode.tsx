...
import { MIXED_TEAM_CODES, type ReviewRow, type CommitPayload } from "@shootermarkt/db/pdf-import-types";
...
export function IssfMode() {
  ...
  interface IssfDirectImportJob {
    id: number;
    status: "queued" | "processing" | "completed" | "failed";
    error: string | null;
    result: { rows: ReviewRow[]; mixedEntries: MixedTeamEntry[]; eventCount: number } | null;
  }
  ...
  useEffect(() => {
    if (!jobId) return;
    const poll = async () => {
      const res = await fetch(`/api/admin/issf/import/${jobId}`);
      const job = await res.json() as IssfDirectImportJob & { error?: string };
      if (!res.ok) throw new Error(job.error ?? "ISSF import nije pronađen");
      if (job.status === "completed" && job.result) {
        setRows(job.result.rows);
        setMixedEntries(job.result.mixedEntries ?? []);
        setEventCount(job.result.eventCount);
        setStep("review");
        setLoading(false);
        setJobId(null);
      } else if (job.status === "failed") {
        setError(job.error ?? "ISSF import nije uspeo");
        setLoading(false);
        setJobId(null);
      }
    };
  ...
  function reset() {
    ...
    setSelectedCodes(new Set()); setSelectedComp(null); setIssfId(""); setJobId(null);
  }
        ...
        <div className="space-y-5">
          ...
          <div className="flex gap-3">
            <button
              ...
              className="rounded-md px-6 py-2.5 text-sm font-semibold text-white bg-[var(--brand-primary)] hover:bg-[var(--brand-primary-hover)] transition-colors disabled:opacity-50"
            >
              {loading ? "Obrađujem…" : `Uvezi ${selectedCodes.size} disciplin${selectedCodes.size === 1 ? "u" : "e"} →`}
            </button>
