...
import type { ReviewRow } from "./pdf-import-types";
...
export const issfDirectImportJobs = pgTable(
  ...
  {
    id: serial("id").primaryKey(),
    input: jsonb("input").$type<{ competitionId: number; disciplineCodes?: string[] }>().notNull(),
    status: varchar("status", { length: 20 }).notNull().default("queued"),
    result: jsonb("result").$type<{ rows: ReviewRow[]; mixedEntries: unknown[]; eventCount: number; mixedErrors: string[] } | null>(),
    error: text("error"),
    attempts: integer("attempts").notNull().default(0),
    createdAt: timestamp("created_at").notNull().defaultNow(),
    startedAt: timestamp("started_at"),
  ...
  (t) => [index("issf_direct_import_jobs_status_idx").on(t.status)]
);
