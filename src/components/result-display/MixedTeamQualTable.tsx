...
export function MixedTeamQualTable({ teams, apparatus, stage = "qual" }: Props) {
  ...
  const columns = ["32px", "minmax(70px, 100px)", "minmax(150px, 1fr)", "72px", "24px", hasSeries ? "18px" : ""].filter(Boolean).join(" ");
  ...
  return <div className="overflow-hidden rounded-xl border border-[var(--border)]">
    ...
    <div className="overflow-x-auto"><div role="table" className="grid text-sm [grid-template-columns:var(--mixed-grid)]" style={{ "--mixed-grid": columns } as CSSProperties}>
      <div role="row" style={{ display: "contents" }}><div role="columnheader" className={`${header} justify-end`}>#</div><div role="columnheader" className={header}>Tim</div><div role="columnheader" className={header}>Strelci</div><div role="columnheader" className={`${header} justify-end`}>Σ</div><div role="columnheader" className={header} aria-hidden="true" />{hasSeries && <div role="columnheader" className={header} aria-hidden="true" />}</div>
