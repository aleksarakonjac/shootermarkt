...
export async function GET(request: NextRequest) {
  ...
  const data = await getHomepageTicker(scope, request.nextUrl.searchParams.get("locale") === "en" ? "en" : "sr");
  return NextResponse.json(data, { headers: { "Cache-Control": "public, s-maxage=60, stale-while-revalidate=300", "Server-Timing": `ticker-db;dur=${(performance.now() - startedAt).toFixed(1)}` } });
}
