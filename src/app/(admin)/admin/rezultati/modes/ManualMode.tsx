...
export function ManualMode() {
  ...
  async function handleSubmit() {
    if (!compName || !compDate) { setError("Naziv i datum takmičenja su obavezni"); return; }
  ...
  function reset() {
    ...
    setCompName(""); setCompDate(""); setCompLocation(""); setCompLevel("national");
    setRows([makeRow()]);
  }
                ...
                >
                  ...
                  <td className={TD}>
                    <input
                      type="number"
                      value={row.qualTotal || ""}
                      onChange={(e) => updateRow(idx, { qualTotal: parseFloat(e.target.value) || 0 })}
                      disabled={row.skip}
                      placeholder="630.5"
                      step="0.1"
                      className={`${INPUT} font-[family-name:var(--font-jetbrains-mono)] text-xs`}
                  ...
                  <td className={TD}>
                    <input
                      type="number"
                      value={row.qualRank ?? ""}
                      onChange={(e) => updateRow(idx, { qualRank: parseInt(e.target.value) || undefined })}
                      disabled={row.skip}
                      placeholder="1"
                      min={1}
                      className={`${INPUT} font-[family-name:var(--font-jetbrains-mono)] text-xs`}
                  ...
                  <td className={TD}>
                    <input
                      ...
                      onChange={(e) => updateRow(idx, { qualInners: parseInt(e.target.value) || null })}
                      disabled={row.skip}
                      placeholder="—"
                      min={0}
                      className={`${INPUT} font-[family-name:var(--font-jetbrains-mono)] text-xs`}
                  ...
                  <td className={TD}>
                    <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button
                        ...
                        title="Dupliraj red"
                        className="w-6 h-6 rounded flex items-center justify-center text-[var(--subtle)] hover:text-[var(--ink)] hover:bg-[var(--surface)] transition-colors"
                      >
                      ...
                      <button
                        ...
                        className="w-6 h-6 rounded flex items-center justify-center text-[var(--subtle)] hover:text-red-500 hover:bg-red-50 transition-colors disabled:opacity-30"
                      >
